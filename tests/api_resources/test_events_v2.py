...
class TestEventsV2:
    ...
    def test_method_get(self, client: Gumnut) -> None:
        events_v2 = client.events_v2.get()
        assert_matches_type(EventsV2Response, events_v2, path=["response"])
    ...
    def test_raw_response_get(self, client: Gumnut) -> None:
        response = client.events_v2.with_raw_response.get()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        events_v2 = response.parse()
        assert_matches_type(EventsV2Response, events_v2, path=["response"])

    @pytest.mark.skip(reason="Prism tests are disabled")
    @parametrize
    def test_streaming_response_get(self, client: Gumnut) -> None:
        with client.events_v2.with_streaming_response.get() as response:
...
class TestAsyncEventsV2:
    ...
    async def test_method_get(self, async_client: AsyncGumnut) -> None:
        events_v2 = await async_client.events_v2.get()
        assert_matches_type(EventsV2Response, events_v2, path=["response"])
    ...
    async def test_raw_response_get(self, async_client: AsyncGumnut) -> None:
        response = await async_client.events_v2.with_raw_response.get()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        events_v2 = await response.parse()
        assert_matches_type(EventsV2Response, events_v2, path=["response"])

    @pytest.mark.skip(reason="Prism tests are disabled")
    @parametrize
    async def test_streaming_response_get(self, async_client: AsyncGumnut) -> None:
        async with async_client.events_v2.with_streaming_response.get() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
